...
for i in range(65,len(filename)):
    rgb.append(folder+'0/rgb/'+ filename[i][:-5] + '.png')
...
cnt=0
record_file = folder+'tfrecords/train_frauas_10classes.tfrecords'
...
cnt=0
record_file = folder+'tfrecords/val_frauas_10classes.tfrecords'
with tf.io.TFRecordWriter(record_file) as writer:
    for p_rgb,p_label,p_x,p_y,p_z in zip(rgb,label,x,y,z):
        cnt+=1
        tf_example = image_example(p_rgb,p_label,p_x,p_y,p_z)
